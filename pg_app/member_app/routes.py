...
def index():
    ...
    define_winner = DefineWinnerForm(liste_id)

    if form.validate_on_submit():
        round_vote = form.round_vote.data
        length_of_selection: int = 8 if round_vote == "2" else 4
        vote_results: list[dict] = render_votes_results(
            int(round_vote), length_of_selection
        )
        return render_template(
            TEMPLATE_MEMBER_INDEX,
    ...
    if define_winner.validate_on_submit():
        winner_id = define_winner.winner.data
        if winner_id:
    ...
    return render_template(
        TEMPLATE_MEMBER_INDEX,
        form=form,
        define_winner_form=define_winner,
        actual_round=actual_round,
    )
    ...
    if request.method == "POST":
        ...
        if confirmation_form is not None:
            ...
            if confirm_vote(id_livres):
                flash(
                    "Les résultats ont été confirmés et enregistrés avec succès.",
                    "success",
                )
            else:
